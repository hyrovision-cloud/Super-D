...
class HttpClient {
  ...
  private getBranchContext(): string | null {
    // UI preference only; the backend independently authorizes this scope.
    return localStorage.getItem('superd_selected_branch');
  }
  ...
  async request<T = any>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    try {
      response = await fetch(url, {
        ...options,
        // credentials: 'include' ensures HttpOnly session cookies are transmitted cross-origin or same-origin
        credentials: 'include',
        headers,
      });
    } catch (err: any) {
