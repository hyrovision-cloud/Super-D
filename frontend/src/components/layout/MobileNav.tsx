...
export const MobileNav: React.FC<{ isOpen: boolean; onOpenChange: (isOpen: boolean) => void }> = ({ isOpen, onOpenChange }) => {
  const { currentRole, permissions, logout } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  const navItems = getAuthorizedItems(permissions);
