...
export const UserListView: React.FC = () => {
  ...
  const canCreate = hasPermission('user.create') && hasPermission('role.assign');
  ...
  const [primaryBranchId, setPrimaryBranchId] = useState('branch-trichy');
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (!name || !email || !employeeId || password.length < 8) {
      toast.error('Enter name, email, employee ID, and a password of at least 8 characters');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleCreateUser} className="space-y-3 text-xs">
          ...
          <div>
            <label className="block font-medium text-text-main mb-1">Initial Password *</label>
            <input
              type="password"
              required
              minLength={8}
