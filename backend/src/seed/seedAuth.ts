...
function normalizedPermissions(roleName: string, permissions: string[]): string[] {
  ...
  if (result.has('leave.request') || result.has('leave.approve')) result.add('leave.view');
  if (result.has('attendance.manage')) { result.add('attendance.update'); result.add('attendance.delete'); }
  if (result.has('settings.view')) result.add('config.view');
  if (result.has('settings.update')) result.add('config.update');
  if (result.has('leave.approve')) result.add('leave.reject');
  if (result.has('user.manage')) { result.add('user.view'); result.add('user.create'); result.add('user.update'); }
  if (result.has('role.manage')) { result.add('role.view'); result.add('role.create'); result.add('role.update'); }
  if (result.has('revenue.view')) result.add('expense.view');
  if (result.has('revenue.create')) { result.add('revenue.update'); result.add('expense.create'); result.add('expense.update'); }
  if (result.has('marketing.view')) result.add('lead.view');
  if (result.has('advertisement.create')) { result.add('advertisement.update'); result.add('lead.create'); result.add('lead.update'); }
  if (result.has('complaint.resolve')) { result.add('complaint.assign'); result.add('complaint.update'); result.add('complaint.close'); }
  if (roleName === 'Hospital Owner' || roleName === 'Super Admin') result.add('dashboard.owner.view');
  return [...result];
}

export async function seedAuthData(): Promise<void> {
  try {
    logger.info('[Seed:Auth] Starting Auth & RBAC seed...');
    ...
    for (const roleDef of INITIAL_ROLES) {
      ...
      await RoleModel.findOneAndUpdate(
        { name: roleDef.name },
        { $set: { ...roleDef, permissions } },
        { upsert: true, new: true }
      );
    }
    logger.info(`[Seed:Auth] Verified ${INITIAL_ROLES.length} system roles.`);

    // 2. Seed All 4 Canonical Hospital Branches
    ...
    for (const branchDef of INITIAL_BRANCHES) {
      await BranchModel.findOneAndUpdate(
        { branchId: branchDef.branchId },
        { $set: branchDef },
        { upsert: true, new: true }
      );
    }
    logger.info(`[Seed:Auth] Verified all ${INITIAL_BRANCHES.length} canonical hospital branches.`);

    // 3. Default password hash for demo accounts
    const defaultPasswordHash = await bcrypt.hash('demo2026@superd', 10);
    ...
    logger.info(`[Seed:Auth] Successfully verified/seeded ${DEMO_USERS.length} demo and test users.`);
  } catch (err: any) {
    logger.error(`[Seed:Auth] Seeding failed: ${err.message}`, err);
    throw err;
  }
}
