...
exports.seedRealData = seedRealData;
exports.verifyRealData = verifyRealData;
...
const isoDay = (offset) => new Date(baseDate.getTime() + offset * 86400000);
...
async function seedRealData() {
    ...
    const revenueCategories = ['OP Consultation', 'Medical / Pharmacy', 'Lab & Diagnostics', 'Day Care', 'Dressing & Procedures'];
    const branchWeights = [1, 1.65, 1.3, 0.78];
    for (let b = 0; b < constants_1.BRANCH_IDS.length; b += 1) {
        const branchId = constants_1.BRANCH_IDS[b];
        const code = constants_1.BRANCH_CODES[branchId];
        const branchName = constants_1.BRANCH_NAMES[branchId];
        ...
        for (let i = 1; i <= 8; i += 1) {
            ...
            income.push({ receiptNumber: `REC-${code}-${String(i).padStart(4, '0')}`, transactionDate: isoDay(i * 2 + 1), category: pick(revenueCategories, i + b), amount, paymentMethod: pick(['Cash', 'UPI', 'Card', 'Net Banking'], i), branchId, patientId: patients.find((x) => x.branchId === branchId && x.uhid.endsWith(String(((i - 1) % patientCount) + 1).padStart(4, '0')))?.uhid, recordedBy: `EMP-${code}-002`, recordedByName: employees.find((x) => x.employeeNumber === `EMP-${code}-002`)?.name || 'Accounts Staff', status: 'ACTIVE' });
        }
        for (let i = 1; i <= 4; i += 1)
            expenses.push({ expenseNumber: `EXP-${code}-${String(i).padStart(4, '0')}`, branchId, category: pick(['Medical Supplies', 'Utilities', 'Maintenance', 'Housekeeping'], i + b), description: 'Verified monthly operating expense', amount: Math.round((900 + i * 430) * branchWeights[b]), expenseDate: isoDay(i * 5), paymentStatus: 'PAID', paymentMethod: pick(['UPI', 'Net Banking', 'Card'], i), recordedByName: 'Finance Team' });
        ...
        for (let i = 1; i <= 3; i += 1)
            leads.push({ leadNumber: `LEAD-${code}-${String(i).padStart(3, '0')}`, name: `${pick(firstNames, b + i + 3)} ${pick(lastNames, i)}`, phone: `+91 7${b + 1}${String(50000000 + i)}`, source: i % 2 ? 'Google Ads' : 'Meta Ads', campaignId: `CAM-${code}-001`, branchId, interestedSpecialty: pick(departments, b + i), status: pick(['NEW', 'CONTACTED', 'APPOINTMENT_BOOKED'], i) });
    }
    await upsertMany(Employee_model_1.EmployeeModel, 'employeeNumber', employees);
    await upsertMany(Patient_model_1.PatientModel, 'uhid', patients);
    await upsertMany(MedicalRecord_model_1.MedicalRecordModel, 'recordNumber', records);
    await upsertMany(Appointment_model_1.AppointmentModel, 'appointmentNumber', appointments);
    for (const item of attendance)
        await AttendanceRecord_model_1.AttendanceRecordModel.updateOne({ employeeId: item.employeeId, date: item.date }, { $set: item }, { upsert: true });
    await upsertMany(LeaveRequest_model_1.LeaveRequestModel, 'requestNumber', leave);
    await upsertMany(Complaint_model_1.ComplaintModel, 'ticketNumber', complaints);
    await upsertMany(IncomeRecord_model_1.IncomeRecordModel, 'receiptNumber', income);
    await upsertMany(Expense_model_1.ExpenseModel, 'expenseNumber', expenses);
    await upsertMany(Advertisement_model_1.AdvertisementModel, 'campaignId', advertisements);
    await upsertMany(Lead_model_1.LeadModel, 'leadNumber', leads);
    const users = await User_model_1.UserModel.find({ status: 'ACTIVE' }).select('_id primaryBranchId').lean();
    await upsertMany(Notification_model_1.NotificationModel, 'notificationKey', users.map((u) => ({ notificationKey: `WELCOME-${u._id}`, userId: String(u._id), branchId: u.primaryBranchId, title: 'Super D data is ready', message: 'Your authorized hospital data is available from the secure API.', type: 'SUCCESS', read: false, link: '/dashboard' })));
    return verifyRealData();
}
async function verifyRealData() {
    const branches = await Branch_model_1.BranchModel.find({ isActive: true }).select('branchId').lean();
    const valid = new Set(branches.map((b) => b.branchId));
    const scopedModels = [Patient_model_1.PatientModel, Employee_model_1.EmployeeModel, Appointment_model_1.AppointmentModel, AttendanceRecord_model_1.AttendanceRecordModel, LeaveRequest_model_1.LeaveRequestModel, Complaint_model_1.ComplaintModel, IncomeRecord_model_1.IncomeRecordModel, Expense_model_1.ExpenseModel, Advertisement_model_1.AdvertisementModel, Lead_model_1.LeadModel];
    for (const model of scopedModels) {
        const orphan = await model.countDocuments({ branchId: { $nin: [...valid] } });
        if (orphan)
            throw new Error(`${model.modelName} has ${orphan} orphan branch references`);
    }
    const counts = { branches: branches.length };
    for (const model of [...scopedModels, MedicalRecord_model_1.MedicalRecordModel, Notification_model_1.NotificationModel])
        counts[model.collection.name] = await model.countDocuments();
    if (branches.length !== 4 || constants_1.BRANCH_IDS.some((id) => !valid.has(id)))
        throw new Error('Canonical four-branch verification failed');
    return counts;
}
